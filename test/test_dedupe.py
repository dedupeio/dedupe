...
class CoreTest(unittest.TestCase):
    def setUp(self):
        ...
        self.ids_str = iter([('1', '2'), ('2', '3'), ('4', '5'),
                             ('6', '7'), ('8', '9')])

        self.records = iter([
            ({'name': 'Margret', 'age': '32'}, {'name': 'Marga', 'age': '33'}),
        ...
        self.normalizedAffineGapDistance = \
            dedupe.affinegap.normalizedAffineGapDistance
        self.data_model = {}
        self.data_model['fields'] = dedupe.core.OrderedDict()
        v = {}
        v.update({'Has Missing': False, 'type': 'String',
                  'comparator': self.normalizedAffineGapDistance,
    ...
    def test_score_duplicates(self):
        ...
        numpy.testing.assert_almost_equal(self.desired_scored_pairs['score'],
                                          scores_str)
        numpy.testing.assert_equal(self.desired_scored_pairs['pairs'],
...
class ConvenienceTest(unittest.TestCase):
    ...
    def test_data_sample(self):
        ...
        with warnings.catch_warnings(record=True) as w:
            ...
            assert len(w) == 1
            assert str(w[-1].message) == "Requested sample of size 10000, " \
                                         "only returning 45 possible pairs"
...
class AffineGapTest(unittest.TestCase):
    ...
    def test_normalized_affine_gap_correctness(self):
        assert numpy.isnan(self.normalizedAffineGapDistance('', '',
                                                            -5, 5, 5, 1, 0.5))
...
class ClusteringTest(unittest.TestCase):
    ...
    def test_hierarchical(self):
        ...
        assert hierarchical(self.dupes, 'i4', 0.5) == \
            [set([1, 2, 3]), set([4, 5])]
        assert hierarchical(self.dupes, 'i4', 0) == \
            [set([1, 2, 3, 4, 5])]
        assert hierarchical(self.str_dupes, 'S1', 1) == []
        assert hierarchical(self.str_dupes, 'S1', 0.5) == \
            [set(['1', '2', '3']), set(['4', '5'])]
        assert hierarchical(self.str_dupes, 'S1', 0) == \
            [set(['1', '2', '3', '4', '5'])]
...
class BlockingTest(unittest.TestCase):
    def setUp(self):
        ...
        self.sameThreeCharStartPredicate = \
            dedupe.predicates.sameThreeCharStartPredicate
        self.training_pairs = {
        ...
        self.predicate_functions = \
            (self.wholeFieldPredicate, self.sameThreeCharStartPredicate)


class PredicatesTest(unittest.TestCase):
    def test_predicates_correctness(self):
        field = '123 16th st'
        assert dedupe.predicates.wholeFieldPredicate(field) == \
            ('123 16th st',)
        assert dedupe.predicates.tokenFieldPredicate(field) == \
            ('123', '16th', 'st')
        assert dedupe.predicates.commonIntegerPredicate(field) == \
            ('123', '16')
        assert dedupe.predicates.sameThreeCharStartPredicate(field) == \
            ('123',)
        assert dedupe.predicates.sameFiveCharStartPredicate(field) == \
            ('123 1',)
        assert dedupe.predicates.sameSevenCharStartPredicate(field) == \
            ('123 16t',)
        assert dedupe.predicates.nearIntegersPredicate(field) == \
            (15, 16, 17, 122, 123, 124)
        assert dedupe.predicates.commonFourGram(field) == \
            ('123 ', '23 1', '3 16', ' 16t', '16th', '6th ', 'th s', 'h st')
        assert dedupe.predicates.commonSixGram(field) == \
            ('123 16', '23 16t', '3 16th', ' 16th ', '16th s', '6th st')
