...
class USAddress(FieldType) :
    ...
    street_name_components = ('StreetNamePreModifier',
                              'StreetName',
                              'StreetNamePostModifier')

    address_number_components = ('AddressNumberPrefix',
                                 'AddressNumber',
                                 'AddressNumberSuffix')

    street_direction_components = ('StreetNamePreDirectional',
    ...
    def comparator(field, field) :
        ...
        if address_type_1 != address_type_2 :
            ...
            if address_type == 'StreetAddress' :
                address_number = concatComponents(address, 
                                                  address_number_components)
                street_direction = concatComponents(address,
                                                    street_directions_components)
                street_name = concatComponents(address, 
                                               street_name_components)
                unit_type = address['OccupancyType']
                unit_id = address['
            address_type
...
def concatComponents(address, components) :
    return ' '.join(address.get(component, '') 
